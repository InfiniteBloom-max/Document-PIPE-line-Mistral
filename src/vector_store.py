...
class VectorStore:
    ...
    def _initialize_embeddings(self):
        ...
        try:
            ...
            st.success(f"Loaded embedding model: {self.embedding_model_name}")
    ...
    def similarity_search(self, query: str, k: int = 5) -> List[Tuple[Document, float]]:
        """Perform similarity search and return top k documents with scores."""
        if self.index is None or not self.documents:
            st.warning("Vector store not initialized. Please upload and process documents first.")
            return []
    ...
    def load_index(self, path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            st.success(f"Vector store loaded from {load_path}")
            return True
